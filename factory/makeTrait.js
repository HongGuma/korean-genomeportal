...
function readVariants(fileName,associationsArr,output){
    ...
    reader.on('line',function(data){
        ...
        for(var ass of associationsArr){
            if(ass.chr == token[0] && ass.pos == token[1] && ass.alt == token[3]){ //association과 variant가 일치하는지
        ...
        cnt++;
        if(cnt % 50000 == 0){ //5만번에 한번씩 점 찍기
            process.stdout.write('='); // 콘솔 출력용
        }
    })

    reader.on('close',function(){ //readline 끝난후
...
function readAssociations(fileName){
    ...
    for(var l of line){
        ...
        arr.push({trait:token[2],chr:`chr${token[4]}`,pos:token[5],alt:token[6]})
...
function mongo(traitArr){
    ...
    db.on('error',function(){ console.log('MongoDB Connection Failed!'); }) // error
    db.once('open',function(){ console.log('MongoDB Connected!'); }) // 연결 성공
    db.collection("trait").insertMany(traitArr,(err,res)=>{ //데이터 insert
