...
route.post('/search',async (req,res)=> {
    try{
        ...
        const geneRes = await db.collection('genelist').find().sort({GeneSymbol:1}).toArray(); //db에서 genelist 불러오기
        const traitRes = await db.collection('trait').aggregate([{$group:{'_id':"$trait"}},{$sort:{'_id':1}}]).toArray(); //db에서 trait 불러오기

        for(let gene of geneRes){ resList.push(gene.GeneSymbol); } //배열에 값 담기
        for(let tr of traitRes){ resList.push(tr._id); } //배열에 값 담기

        if(keyword != null && keyword != ''){ //입력받은 키워드가 있다면
